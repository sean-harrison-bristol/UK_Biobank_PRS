##########################################################################################
#Script name: mrbase_grs_v2.07.R
#Project: GRS_SEP
...
#Update 3.01: Updated to work on either Blue Crystal 3 or 4, with the default as Blue Crystal 4
#Update 2.07: Updated the way the IPD is created - now it outputs as a .raw file
#Update 2.06: Updated to force the clumping options to work with MR Base, and to remove intermediate clumping files
#Update 2.05: Updated the proxies code to work when proxies are found, but none are in UK Biobank
#Update 2.04: Updated the plink_grs option to work with the X chromosome
#Update 2.03: Updated the clumping code to have a default 10,000 kb window with an R2 of 0.001, same as MR Base, and added options to allow changes
...
mrbase_grs = function(output=NULL,category=NULL,subcategory=NULL,trait=NULL,population=NULL,sex=NULL,mr=NULL,samplesize=0,notstudies=NULL,studies=NULL,p=5e-08,
                      exposure_dat=NULL,exposure_file=NULL,
                      proxies=TRUE,r2=0.8, 
                      clump=TRUE,clumped=TRUE,clump_kb=10000,clump_r2=0.001,
                      gwas="biggest",suffix="",
                      snpstats_file="SNPstats.txt",ld_file="1000_genomes_ld.csv.gz",eur_file = "eur",
                      keep_files=FALSE,
                      ipd=FALSE,
                      maf=0.42,
                      plink_grs=FALSE,
  ...
  } else if(bc == 4 & bgen_folder == ""){
    bgen_folder="/mnt/storage/private/mrcieu/data/ukbiobank/genetic/variants/arrays/imputed/released/2018-09-18/data/dosage_bgen"
  }
  ...
  ###################SNPS, Code, GRS##################################    
    ...
    if(tolower(output)=="snps"){
      ...
      if(j > 0){
        mrbase_grs_instruments = extract_instruments(outcomes=mrbase_grs_studies, p1 = p, "clump" = clump, p2 = p,"kb" = clump_kb,"r2" = clump_r2)
        print("Processing complete")
    ...
    ###################Code##################################  
      ...
      if(is.null(exposure_file) & is.null(exposure_dat)){
        ...
        if(j > 0){
          ...
          for(study in mrbase_grs_studies){
            ...
            temp = try(extract_instruments(outcomes=study, p1 = p, "clump" = clump, p2 = p,"kb" = clump_kb,"r2" = clump_r2),silent=TRUE)
      ...
      snp_ipd_file = paste("snp_ipd",suffix,sep="")
      ...
      if(clumped == FALSE | "clumped" %in% names(exposure_dat)){
        ...
        write(paste('for(t in trait){
                    ...
                    write.table(unclumped_snps,"unclumped_snps.txt",row.names=FALSE,quote=FALSE)',sep=""),file,append=TRUE)
        
        #BC3 versus BC4            
        if(bc == 3) {        
          write(paste('system(\'module rm apps/plink-2.00
                    module add apps/plink-1.90b4.1
                    plink -bfile ',eur_file,' --clump unclumped_snps.txt --clump-kb ',clump_kb,' --clump-p1 1 --clump-p2 1 --clump-r2 ',clump_r2,' --out clumped_snps
                    tr -s [:blank:] < clumped_snps.clumped | cut -f 4 -d " " > clumped_snps.txt
                    module rm apps/plink-1.90b4.1
                    module add apps/plink-2.00\')',sep=""),file,append=TRUE)
        } else if(bc == 4) {
          write(paste('system(\'module rm apps/plink/2.00
                    module add apps/plink/1.90
                    plink -bfile ',eur_file,' --clump unclumped_snps.txt --clump-kb ',clump_kb,' --clump-p1 1 --clump-p2 1 --clump-r2 ',clump_r2,' --out clumped_snps
                    tr -s [:blank:] < clumped_snps.clumped | cut -f 4 -d " " > clumped_snps.txt
                    module rm apps/plink/1.90
                    module add apps/plink/2.00\')',sep=""),file,append=TRUE)
        }
                    
        write(paste('#Then bring the SNP list back into R & remove pruned SNPs
      ...
      if(bc == 3) { 
        ...
        l1 = "system('folder=/mnt/storage/private/mrcieu/data/ukbiobank/genetic/variants/arrays/imputed/released/2018-09-18/data/snp-stats"
      }
      ...
      if(proxies == TRUE) { 
        ...
        #The included_proxy_SNPs_file could also be blank
        write(paste("system('wc -l ",included_proxy_SNPs_file," > ",snp_ld_test_file,"')",sep=""),file,append = TRUE)
        write(paste("ld_x = read.delim('",snp_ld_test_file,"',header=FALSE,stringsAsFactors = FALSE, strip.white=TRUE,sep=' ')",sep=""),file,append = TRUE)
        write("if(ld_x$V1[1] != 0)  {",file,append = TRUE)
        ...
        write("print('No proxies in UK Biobank found, continuing')",file,append=TRUE)
      ...
      #Convert BGEN to Raw file if using R to create PRS or IPD == TRUE
      if(plink_grs == FALSE | ipd == TRUE) {
        write(paste('plink2 --bgen ',instruments_file,'.bgen --hard-call-threshold 0.4999 --export A --out ',snp_ipd_file,sep=""),file,append=TRUE)
      }
      write("')",file,append=TRUE)
      
      if(plink_grs == FALSE){  
        write(paste('snp_ipd = as.data.frame(fread("',snp_ipd_file,'.raw", header = T, sep = "\t"))',sep=""),file,append=TRUE)
  ...
  #In some cases, it will be easier to create the GRS using plink
  ...
  #Plink takes more time to look through all the SNPs in each chromosome than Bgenix does, so it's quicker to use Bgenix first, then Plink
  write("system('",file,append=TRUE)
  write(paste('  plink --bgen ',instruments_file,'.bgen --score ',snp_score_list_file,' --out ',instruments_file,sep=""),file,append=TRUE)
  write("')",file,append=TRUE)
  
  write(paste('temp = read.delim("',instruments_file,'.sscore")',sep=""),file,append=TRUE)
  ...
  write(paste('write.csv(grs,"',grs_file,'",row.names=FALSE)',sep=""),file,append=TRUE)
      ...
      if(keep_files==FALSE){
        ...
        snp_ipd_log_file = paste(snp_ipd_file,".log",sep="")
        instruments_raw_file = paste(instruments_file,".raw",sep="")
        write(paste("system('rm",snp_list_file,included_SNPs_file,paste(instruments_file,".bgen",sep=""),"')",sep=" "),file,append=TRUE)
        write(paste('system(\'if [ -f "',snp_list_out_file,'" ]
                    then
                    rm ',snp_list_out_file,'
                    fi
                    if [ -f "unclumped_snps.txt" ]
                    then
                    rm  unclumped_snps.txt                   
                    fi
                    if [ -f "clumped_snps.log" ]
                    then
                    rm  clumped_snps.log
                    fi
                    if [ -f "clumped_snps.nosex" ]
                    then
                    rm  clumped_snps.nosex
                    fi
                    if [ -f "clumped_snps.clumped" ]
                    then
                    rm  clumped_snps.clumped
                    fi
                    ...
                    if [ -f "',instruments_raw_file,'" ]
                    then
                    rm ',instruments_raw_file,'
                    fi
                    if [ -f "',snp_ld_file,'" ]
                    then
                    rm ',snp_ld_file,'
                    ...
                    if [ -f "',instruments_file,'.sscore" ]
                    then
                    rm  ',instruments_file,'.sscore
                    fi
                    if [ -f "',snp_ipd_log_file,'" ]
                    then
                    rm  ',snp_ipd_log_file,'
                    fi
      ...
      if(bc == 3) { 
        ...
        file = run_file
        write("#!/bin/bash",file,append = FALSE)
        write("#SBATCH --nodes=1",file,append=TRUE)
        write("#SBATCH --ntasks-per-node=1",file,append=TRUE)
        write("#SBATCH --cpus-per-task=1",file,append=TRUE)
        write("#SBATCH --time=1-00:00:00",file,append=TRUE)
        write("#SBATCH --job-name=GRS_creation",file,append=TRUE)
        write("#SBATCH -o GRS_creation_output",file,append=TRUE)
        write("#SBATCH -e GRS_creation_error",file,append=TRUE)
        write("#SBATCH --mem=64000M",file,append=TRUE)
        write("#SBATCH --partition=hmem",file,append=TRUE)
        
        write("cd $SLURM_SUBMIT_DIR",file,append=TRUE)
        write("",file,append=TRUE)
        write("module load apps/bgen/1.1.4",file,append=TRUE)
        write("module load apps/plink/2.00",file,append=TRUE)
        write("module load languages/r/3.6.2",file,append=TRUE)
        write("",file,append=TRUE)
        write(paste("srun R --no-save < ./",script_file,sep=""),file,append=TRUE)
      }
      ...
      if(bc == 4){
        print(paste("You can submit the scripts to the queue on Blue Crystal 4, type: sbatch ",run_file,sep=""))
      }
